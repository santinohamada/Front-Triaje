...
import {
  REGISTRO_INGRESO_DEFAULT_VALUES,
  registroIngresoSchema,
  RegistroIngresoSchema,
} from "@/schemas/registroIngreso.schema";
...
export function EnfermeroDashboard() {
  ...
  const handleBuscarPaciente = async () => {
    ...
    try {
      const { data } = await api
        .get(`/api/ingreso/buscar-por-cuil?cuil=${cuil}`)
        .catch((err) => {
      ...
      if (data) {
        ...
        toastUtils.success(
          "Paciente nuevo",
          "No se encontró en el sistema. Complete los datos."
        );
  ...
  const onSubmit = async (values: RegistroIngresoSchema) => {
    ...
    const dto: RegistroIngresoRequest = {
      ...
      nombreObraSocial: values.tieneObraSocial
        ? values.nombreObraSocial
        : undefined,
      numeroAfiliado: values.tieneObraSocial
        ? values.numeroAfiliado
        : undefined,
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
            <FormField
              ...
              name="cuilPaciente"
              render={({ field }) => (
                // 1. Agregamos "relative" y quizás "pb-6" si quieres reservar el espacio visualmente
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 bg-background">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>
                        Activar si el paciente posee cobertura
                      </FormDescription>
                    </div>
                    <FormControl>
                      <Switch
                        ...
                        disabled={
                          patientFound &&
                          form.getValues("nombreObraSocial") !== ""
                        }
                      />
                    </FormControl>
                  </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="sm:col-span-3 lg:col-span-1">
                      ...
                      <Select
                        onValueChange={(v) =>
                          field.onChange(Number(v) as PrioridadTriaje)
                        }
                      >
                        <FormControl>
                        ...
                        <SelectContent>
                          <SelectItem value={String(PrioridadTriaje.Critico)}>
                            🔴 Crítico
                          </SelectItem>
                          <SelectItem
                            value={String(PrioridadTriaje.Emergencia)}
                          >
                            🟠 Emergencia
                          </SelectItem>
                          <SelectItem value={String(PrioridadTriaje.Urgencia)}>
                            🟡 Urgencia
                          </SelectItem>
                          <SelectItem
                            value={String(PrioridadTriaje.UrgenciaMenor)}
                          >
                            🟢 Urgencia Menor
                          </SelectItem>
                          <SelectItem
                            value={String(PrioridadTriaje.SinUrgencia)}
                          >
                            🔵 Sin Urgencia
                          </SelectItem>
                        </SelectContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormMessage className="h-2 text-xs" />
                  </FormItem>
                )}
              />
            </div>
            ...
            <div >
              ...
              <Button
                ...
                disabled={registrarIngresoMutation.isPending}
              >
                {registrarIngresoMutation.isPending ? (
                  ...
                  <>
                    <CheckCircle2 className="h-5 w-5" /> Confirmar y Registrar
                    Ingreso
                  </>
                )}
              </Button>
